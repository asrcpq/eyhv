...
// font resources, 10.0x10.0 vector
lazy_static! {
    static ref FONT_DATA: HashMap<char, GraphicObjects> = "1 "
        .chars()
        .map(|c| (
            c,
            match c {
                '1' => GraphicObjects::from_strs(vec![
                    "l 1 1 1 1 5.5 1 3.5 2.5 4 3 4.5 2.5 4.5 9 5.5 9 5.5 1",
                ]).zoom(0.1),
                ' ' => Default::default(),
                _ => unreachable!(),
            }
...
pub struct Background {
    lines_h: VecDeque<Vec<[f32; 3]>>,
    lines_v: VecDeque<Vec<[f32; 3]>>,
    lines_font: VecDeque<Vec<[f32; 3]>>,
...
impl Background {
    ...
    pub fn tick(&mut self, dt: f32, slowing: bool) {
        ...
        for line in self.lines_font.iter_mut() {
            for vertex in line.iter_mut() {
                vertex[0] -= dt * SPEED_SCALER_H;
                vertex[1] -= dt * SPEED_SCALER_V;
            }
        ...
        if self.timer_v >= 1. / SPEED_SCALER_V {
            if let Some(string) = self.message_queue.pop_front() {
                ...
                for ch in string.chars() {
                    ...
                    if let Some(graphic_objects) = FONT_DATA.get(&ch) {
                        for graphic_object in graphic_objects.clone().into_iter() {
                            if let Some(line_segs) = graphic_object
                                .as_any()
                                .downcast_ref::<LineSegs2f>() {
                    ...
                    self.lines_font.push_back(vertices);
    ...
    pub fn graphic_objects_iter(&self) -> GraphicObjectsIntoIter {
        ...
        for layer in 0..5 {
            'line_segs: for line in match layer {
                0 => self.lines_h.iter(),
                1 => self.lines_v.iter(),
                2 => self.lines_h.iter(),
                3 => self.lines_v.iter(),
                ...
                for vertex in line_tmp.iter_mut() {
                    vertex[0] *= 10.;
                    vertex[1] *= 10.;
                    for i in 0..3 {
                        vertex[i] += line_shift[layer][i];
                    }
                }
